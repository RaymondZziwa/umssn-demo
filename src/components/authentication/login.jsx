    ...
    const tutorLoginHandler = async (event , tutorIdNumber, tutorPassword) => {
        ...
        if(tutorIdNumber.trim().length !== 0 && tutorPassword.trim().length !== 0){
            ...
            if(res.data.responseStatus !== 404){
                ...
                localStorage.setItem('tutorIdNumber', res.data.tutorIdNumber)
                localStorage.setItem('userLoginStatus', true)
                navigate(`${res.data.redirectPath}`)
            }else{
                setIsLoginSuccessful(false)
    ...
    return(
        <div className="container min-vh-100 d-flex justify-content-center align-items-center">
            <Form className="login-form">
                <div className="mb-3">
                    <img src={logo} alt="logo" height="180px" style={{marginLeft:'45px'}}/>
                ...
                {role === 'teacher' && 
                    <TeacherLogin loginHandler={tutorLoginHandler}/>
                }
                {role === 'student' &&
                    <StudentLogin loginHandler={loginHandler}/>
                }
                <div className="mb-3">
                    <p style={{ textAlign: "center", fontSize: "18px", marginTop: "10px", color: "#3E5AA7" }}><Link to="/register">Are you a new parent? Activate here.</Link></p>
                    <p style={{ textAlign: "center", fontSize: "18px", marginTop: "10px", color: "#3E5AA7" }}><Link to="/teacheractivation">Are you a new tutor? Activate here.</Link></p>
                </div>
                <div className="mb-3">
